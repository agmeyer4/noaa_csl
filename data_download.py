...
class NOAA_CSL_download:
    ...
    def retrieve_format_data(self,sector,year,month):
        ...
        '''
        month_str = self.month_int_to_str(month) #convert the integer month into the month string used by NOAA
    ...
    def dir_dict_setup(self,sector,year_str,month_str,download_path):
        ...
        prefix = replace_all_strs(prefix_temp,{'MonthXX':month_str}) #get the real prefix by replacing the month (if necessary)
...
# Main Function
def main():
    base_data_dir = '/uufs/chpc.utah.edu/common/home/lin-group9/agm/NOAA_CSL' #where the data will be stored
    check_space(base_data_dir,excep_thresh='6Tb') #ensure there is enough space in the director
    ncd = NOAA_CSL_download(base_data_dir,bau_or_covid='COVID') #setup the downloader
